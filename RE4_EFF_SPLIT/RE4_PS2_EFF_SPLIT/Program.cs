            ...
            else if (args.Length >= 1 && File.Exists(args[0]))
            {
                ...
                if (fileInfo != null)
                {
                    ...
                    if (fileInfo.Extension.ToUpperInvariant() == ".EFF")
                    {
                        try
                        {
                            EFF_SPLIT.Extract.ExtractFilePS2(fileInfo.FullName);
                        }
                        catch (Exception ex)
                        {
                    ...
                    else if (fileInfo.Extension.ToUpperInvariant() == ".IDX_PS2_EFF_SPLIT")
                    {
                        try
                        {
                            EFF_SPLIT.Repack.RepackFilePS2(fileInfo.FullName);
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine("Error: " + Environment.NewLine + ex);
