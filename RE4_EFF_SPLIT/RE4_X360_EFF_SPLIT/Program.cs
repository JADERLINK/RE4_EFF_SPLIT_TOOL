...
namespace RE4_X360_EFF_SPLIT
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("# RE4 X360 EFF SPLIT");
                    ...
                    if (File.Exists(args[i]))
                    {
                        ...
                        if (fileInfo != null)
                        {
                            ...
                            if (fileInfo.Extension.ToUpperInvariant() == ".EFF")
                            {
                                try
                                {
                                    EFF_SPLIT.Extract.ExtractFileX360(fileInfo.FullName);
                                }
                                catch (Exception ex)
                                {
                            ...
                            else if (fileInfo.Extension.ToUpperInvariant() == ".IDX_X360_EFF_SPLIT")
                            {
                                try
                                {
                                    EFF_SPLIT.Repack.RepackFileX360(fileInfo.FullName);
                                }
                                catch (Exception ex)
                                {
                                    Console.WriteLine("Error: " + Environment.NewLine + ex);
